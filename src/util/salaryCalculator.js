...
// Usage example
try {
  const baseSalary = calculateProposedSalary({
    value: 3000.5,
    currencyCode: "EUR",
    period: "monthly",
  });

  const performanceBonus = { value: 500, currencyId: "978" }; // Performance bonus
  const signOnBonus = { value: 1000, currencyId: "978" }; // Welcome bonus
  const retentionBonus = { value: 200, currencyId: "978" }; // Retention bonus

  const totalSalary = calculateTotalSalary(baseSalary, [
    performanceBonus,
    signOnBonus,
    retentionBonus,
  ]);

  console.warn("Base Salary:", baseSalary);
  console.warn("Total Salary:", totalSalary);
} catch (error) {
